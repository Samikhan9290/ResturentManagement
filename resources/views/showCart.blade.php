...
@endif
<div id="top">
    <table border="1px" align="center" width="500px" style="background-color: lightcyan">
        <tr>
        ...
        <form action="{{url('confirmOrder',Auth::user()->id)}}" method="post">
            @csrf
        @foreach($data as $list)
        <tr>
            <td>
                <input type="text" name="foodName[]" value="{{$list->title}}" hidden>
                {{$list->title}}
            ...
            <td>
                ...
                {{$list->price}}</td>
            <td>
                <input type="text" name="quantity[]" value="{{$list->quantity}}" hidden>
                {{$list->quantity}}</td>
{{--            <td>{{$list->id}}</td>--}}
        ...
        <tr style="position: relative;top: -30px;right: -360px">
            <td><a class="btn btn-primary" href="{{url('/remove/'.$list->id)}}">Remove</a></td>
        </tr>
        @endforeach
    </table>
</div>
